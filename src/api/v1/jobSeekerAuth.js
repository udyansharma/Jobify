...
router.post("/signIn", async (req, res, next) => {
    try {
        ...
        let fetchedjobSeeker = await jobSeeker.signIn(req.body);
        let fetchedjobSeekerEmail = fetchedjobSeeker.email;
        let token = jwt.sign({ fetchedjobSeekerEmail }, config.application.signature, { expiresIn: config.application.jwtexpiration });
        let allJobs = await jobSeeker.getEntireJobList();
        return res.status(200).render("pages/jobseeker.ejs", { token: token, title: "All Jobs", data: allJobs });
    }
    catch (err) {
        console.log(err);
        if (err == "Server Error") {
